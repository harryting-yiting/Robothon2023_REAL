...
#-------------------------------Tasks--------------------------------#  
    ...
    def task_slider_move_back_force_x(self, flange_to_tcp_up,flange_to_tcp_down, slider_init_tcp,target_dis_to_up, back_force_dis, speed, acc):
        ...
        self.rtde_c.moveL(tmToXyzRA(slider_target_force),self.default_speed, self.default_acc)
        for i in range(0, 1):
    ...
    def task_move_slider(self):
        ...
        move_slider_acc = 0.2
        # move to mid 
        self.rtde_c.moveL(tmToXyzRA(slider_mid_tcp), self.default_speed, self.default_acc)
        time.sleep(1)
        ...
        if resp1.results.flag is 2:
            ...
            self.rtde_c.moveL(tmToXyzRA(transfIntrinsric( slider_init_tcp, [0.015, 0, 0, 0, 0, 0])),self.default_speed, self.default_acc)
            self.task_slider_move_back_force_x(flange_to_tcp_up,flange_to_tcp_down, slider_init_tcp,0.015, 0.006, move_slider_speed, move_slider_acc)
            self.rtde_c.setTcp(flange_to_tcp_up)
            self.rtde_c.moveL(tmToXyzRA(moveAbove(tcp_poses_rotated[2], self.default_hight)),self.default_speed, self.default_acc)
        else:
            print("detected")
            ...
            # move to recognised 
            self.rtde_c.moveL(tmToXyzRA(transfIntrinsric( slider_init_tcp, [x, 0, 0, 0, 0, 0])),self.default_speed, self.default_acc)
            time.sleep(1)
...
if __name__ == "__main__":
    ...
    while not rospy.is_shutdown():
        ...
        if recived:
        #if False:
            boardBaseXyzRpy = np.array([1e-3 * boardPose.x, 1e-3 * boardPose.y, 0.0605, 0, 0, math.radians(boardPose.theta)])
